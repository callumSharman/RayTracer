...
int main() {
    ...
    material_t center_mat = lamb_surface_init(vec3_init(0.1,0.2,0.5));
    ...
    // LEFT SPHERE
    sphere_list.spheres[3] = sphere_init(vec3_init(-1.0,0.0,-1), -0.4, left_mat);

    // RIGHT SPHERE
    sphere_list.spheres[4] = sphere_init(vec3_init(1.0,0.0,-1), 0.5, right_mat);
