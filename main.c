...
int main() {
        ...
        for(int j = -7; j < 7; j ++){
            ...
            if(vec3_mag(vec3_sub(center, vec3_init(4,0.2,0))) > 0.9){
                if(choose_mat < 0.7){
                    ...
                    sphere_list.spheres[list_idx] = sphere_init(center, 0.2, mat);
                    list_idx ++;
    ...
    sphere_list.spheres[list_idx] = sphere_init(vec3_init(0.0,1,1.1), 1.0, mat1);
    list_idx ++;

    sphere_list.spheres[list_idx] = sphere_init(vec3_init(0.0,1,-1.1), 1.0, mat1);
    list_idx ++;
    sphere_list.spheres[list_idx] = sphere_init(vec3_init(0.0,1,-1.1), -0.8, mat1);
    list_idx ++;
